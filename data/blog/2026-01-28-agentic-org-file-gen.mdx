---
title: 'Managing my org files with agents'
date: '2026-01-28'
...
;;; Org Agenda on Startup (Doom Emacs)
  ...
  (run-with-timer 300 300 #'my/agenda-refresh)
...
It's so good. I can view an individual item to get a link right to the Linear issue / Github, easily toggle between `TODO`/`IN REVIEW`/`WAITING` modes, filter by status or tag, etc. I can also call the `my/agenda-refresh` function defined there ^ if I don't want to wait for the every 5 min refresh.
