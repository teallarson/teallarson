...
const Card = ({ title, description, imgSrc, href, linkText }: CardProps) => (
  <div className="group md p-4 md:w-1/2" style={{ maxWidth: '544px' }}>
    ...
    >
      ...
      <div className="absolute -inset-0.5 rounded-xl bg-gradient-to-r from-primary-500 via-primary-400 to-primary-600 opacity-0 blur transition-opacity duration-300 group-hover:opacity-30 dark:group-hover:opacity-40 hotdog:hidden"></div>
      {imgSrc &&
        (href ? (
